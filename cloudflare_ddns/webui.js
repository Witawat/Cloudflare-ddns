...
const I18N = {
  th: {
    ...
    "tunnel.connect_timeout_hint": "เวลารอเชื่อมต่อ origin — origin ช้า/หลุดบ่อย ให้เพิ่มค่า",
    "tunnel.origin_server_name_hint": "ใช้เมื่อ cert ของ origin ออกให้ชื่ออื่น (CN ไม่ตรง IP) — ใส่ชื่อที่ cert ออกให้ เช่น nas.local",
    "tunnel.keep_alive_timeout_hint": "เวลาที่จะเก็บ connection ไป origin — ลดค่าถ้า origin จำกัด connection พร้อมกัน",
    "tunnel.keep_alive_connections_hint": "จำนวน connection ที่เก็บไว้พร้อมกัน — ลดถ้า origin รองรับน้อย (เช่น dev server)",
    "tunnel.no_chunked_encoding_hint": "ใช้เมื่อ origin เป็น WSGI (Flask/Django dev server) แล้วหน้าเว็บค้าง/โหลดไม่ขึ้น",
    "tunnel.tls_timeout_hint": "เวลารอ TLS handshake กับ origin (https) — origin ช้า/เครื่องเก่า ให้เพิ่มค่า",
    "tunnel.http2_origin_hint": "ใช้เมื่อ origin รองรับ HTTP/2 (เช่น nginx เปิด h2) — ลด latency. เฉพาะ http/https",
    "tunnel.no_happy_eyeballs_hint": "ใช้เมื่อ origin มี IPv6 ที่ค้าง/ไม่ตอบ — บังคับใช้ IPv4. เฉพาะ http/https",
  ...
  en: {
    ...
    "tunnel.connect_timeout_hint": "Time to wait for the origin connection — increase if the origin is slow/flaky",
    "tunnel.origin_server_name_hint": "Use when the origin cert is issued to another name (CN doesn't match the IP) — set the name on the cert, e.g. nas.local",
    "tunnel.keep_alive_timeout_hint": "How long to keep connections to the origin — lower it if the origin limits concurrent connections",
    "tunnel.keep_alive_connections_hint": "How many connections to keep open — lower if the origin supports few (e.g. dev server)",
    "tunnel.no_chunked_encoding_hint": "Use when the origin is WSGI (Flask/Django dev server) and pages hang/won't load",
    "tunnel.tls_timeout_hint": "Time to wait for the TLS handshake with the origin (https) — increase for slow/old machines",
    "tunnel.http2_origin_hint": "Use when the origin supports HTTP/2 (e.g. nginx with h2) — lower latency. http/https only",
    "tunnel.no_happy_eyeballs_hint": "Use when the origin has a stuck/unresponsive IPv6 — forces IPv4. http/https only",
