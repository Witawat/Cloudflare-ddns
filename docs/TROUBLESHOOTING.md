...
- รหัสคือค่าที่ตั้งในฟอร์ม (`webui_password`) — เก็บเป็น hash อยู่แล้ว (ดูค่าใน config เป็น hash ไม่ใช่รหัสจริง)
- ผิดเกิน 5 ครั้งติดต่อกัน → ล็อกชั่วคราว 5 นาที (ขึ้นข้อความ "ล็อกชั่วคราว") — รอครบเวลาแล้วลองใหม่ (restart service = ปลดล็อกทันที)

### ลืมรหัสผ่านหน้าเว็บ (กู้ได้ 3 ทาง)
1. **ยังไม่หลุดออกจากหน้าเว็บ** → ตั้งค่า → เปลี่ยนรหัสในช่อง "รหัสผ่านหน้าเว็บ" → บันทึก (session เก่าใช้ได้จนกว่าจะปิด)
2. **ใช้คำสั่งในเครื่อง** (ต้องรู้รหัส Windows) → เปิด cmd/terminal แล้วรัน: `cloudflare-ddns.exe reset-password` (ตั้งรหัสใหม่ หรือเว้นว่าง = ลบรหัส ไม่ต้อง login) — ใช้ได้กับ exe ที่กำลังรันอยู่
3. **กู้ผ่าน Telegram** (ต้องเปิดไว้ก่อนล่วงหน้า) → ในแชทกับ bot พิมพ์ `reset password` → โปรแกรมตอบกลับ → พิมพ์ `yes` → ได้รหัสใหม่สุ่ม 12 ตัว (เปิดได้ในฟอร์มตั้งค่า "ควบคุม/กู้รหัสผ่านผ่าน Telegram" หรือ `telegram_allow_reset = true` ใน config — เฉพาะ chat_id ที่ตั้งไว้ กัน 1 ครั้ง/10 นาที — ดูคำสั่งอื่นได้ด้วยการพิมพ์ `/help`)
- ทางเลือกสุดท้าย (รู้จัก config) → แก้ `webui_password =` ใน config.ini ให้ว่าง แล้ว restart — เข้าเว็บได้โดยไม่ต้อง login
