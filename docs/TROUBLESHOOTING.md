...
3. **กู้ผ่าน Telegram** (ต้องเปิดไว้ก่อนล่วงหน้า) → ในแชทกับ bot พิมพ์ `reset password` → โปรแกรมตอบกลับ → พิมพ์ `yes` → ได้รหัสใหม่สุ่ม 12 ตัว (เปิดได้ในฟอร์มตั้งค่า "กู้รหัสผ่านหน้าเว็บผ่าน Telegram" หรือ `telegram_allow_reset = true` ใน config — เฉพาะ chat_id ที่ตั้งไว้ กัน 1 ครั้ง/10 นาที)
