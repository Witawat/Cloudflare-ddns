...
**ใช้ bot ตัวเดียวร่วมหลายเครื่อง:** ตั้งชื่อเครื่องในฟอร์มตั้งค่า (ช่อง "ชื่อเครื่องรับคำสั่ง") หรือใส่ `telegram_command_name` ใน config.ini — จากนั้นสั่ง `/status @ชื่อเครื่อง` ได้ (เฉพาะเครื่องที่ชื่อตรงตอบ) — ถ้าไม่ระบุ `@ชื่อ` คำสั่งจะส่งถึงทุกเครื่อง (ทุกตัวตอบ) — คำตอบทุกรายการขึ้นต้นด้วย `[ชื่อ]` และการแจ้งเตือนลงท้ายด้วย `· ชื่อ`
...
- **ตั้งค่า**: ฟอร์ม (token/interval/password/พอร์ต/**host ที่เปิด**/log/Heartbeat/กัน CF IP/**ฉันทามติ IP**/Tunnel/Telegram/records) + โหมด "แก้ไขไฟล์โดยตรง" (textarea + ตรวจ syntax) + auto-backup config (เก็บ 5 อัน) + **ดาวน์โหลด/นำเข้า config.ini** (สำรอง/ย้ายเครื่อง) — เปิดจากเครื่องอื่นใน LAN ได้ (`webui_host = 0.0.0.0` + รหัสผ่าน + firewall)
