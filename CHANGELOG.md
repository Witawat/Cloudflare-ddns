...
- **หัวเว็บ (topbar) ล้นขอบบนมือถือ**: จอ ≤560px — brand + ปุ่ม 6 รายการ (สถานะ/เวอร์ชัน/ภาษา/
  ขนาดอักษร/รีเฟรช) กว้างเกินจอ → ล้นแนวนอน แก้: ลด padding, จัด 2 แถว
  (แถว 1: สถานะ + รีเฟรช · แถว 2: ภาษา + A−/A+)
- **มือถือ "รอบล่าสุด" หาย + ล้นที่ 320px**: `.brand .sub` (อัปเดต IP อัตโนมัติ · รอบล่าสุด \<เวลา\>)
  เป็นข้อมูลสำคัญ ต้องโชว์ — แก้ให้แสดง (ย่อขนาด + ellipsis ตัดเมื่อยาว) ไม่ซ่อน —
  ยืนยัน 320/360/1920px ไม่ล้นทั้ง topbar

### เพิ่ม (Tests)

- (175 เทสต์ — ตรวจผ่าน playwright จอ 320/360/1920)
...
- **ปุ่มปรับขนาดหน้าเว็บ A−/A+** ที่หัวเว็บ (ข้างปุ่มภาษา): 3 ระดับ 100% / 125% / 150%
  — ใช้ CSS `zoom` ขยาย**ทั้งหน้า** (ตัวอักษร + กรอบ + ระยะห่าง + เลย์เอาต์) เหมือน browser zoom
  จริง (125% = zoom 125%) — จำค่าไว้ใน localStorage (cfddns_fs)
  - พื้นฐานเดิม: ขนาดอักษรปรับตามความกว้างจออัตโนมัติอยู่แล้ว (clamp 14-17px)
  - ตรวจผ่าน playwright: zoom 1 → main 860px · zoom 1.25 → main 1075px (×1.25) — กรอบโตตาม

### เพิ่ม (Tests)

- (170 เทสต์ — ตรวจผ่าน playwright: 17px → 21.25px (125%) → 25.5px (150%) + จำค่า reload)
...
- **Banner เปิดตัวบน console**: แสดงชื่อโปรแกรม + เวอร์ชัน + เครดิตผู้พัฒนา (MAKER WITAWAT · github.com/Witawat/Cloudflare-ddns) ทุกครั้งที่เปิดโปรแกรม (เปิด exe / `run` / `webui`)

### แก้บั๊ก (Fixes)

- **กัน reset รหัสผ่านหน้าเว็บโดนเผลอ**: พิมพ์ `/run` / `/restart` / `/tunnel stop` ขณะที่ `reset password` ยังค้างรอยืนยัน → ระบบยกเลิก reset อัตโนมัติ (ก่อนหน้านี้พิมพ์ `yes` ซ้ำหลังยืนยันคำสั่งอันตราย = reset รหัสโดยไม่ตั้งใจ)
- **คำสั่งใหม่ไม่ถูกกลืนหลังยืนยันหมดเวลา**: `/run` ทิ้งไว้จนหมด 2 นาที แล้วพิมพ์คำสั่งอื่น → คำสั่งนั้นถูกประมวลผลปกติ (ก่อนหน้านี้ตอบ "มีคำสั่งรอยืนยัน" ทั้งที่ state ถูกล้างแล้ว + คำสั่งหาย) — พิมพ์ `yes`/`no` หลังหมดเวลา = ตอบ "หมดเวลายืนยันแล้ว" ไม่รัน
...
- **ระบุชื่อเครื่องในคำสั่ง Telegram** (ใช้ bot ตัวกลางร่วมหลายเครื่อง): พิมพ์ `/status @ชื่อเครื่อง` — เฉพาะเครื่องที่ชื่อตรงตอบ ที่เหลือข้าม (ชื่อตั้งได้ในฟอร์มตั้งค่า/`telegram_command_name` — เว้นว่าง = ชื่อเครื่องของระบบ)
- **โปรโตคอลกันขโมยคำสั่งระหว่างเครื่อง** (bot เดียว หลาย instance): คำสั่ง `@เครื่องอื่น` ไม่ถูก confirm offset (ยังรอคิวให้เครื่องเป้าได้รับเอง) — ถ้าเครื่องเป้าออฟไลน์เกิน 10 นาที ทิ้งไป ไม่บล็อกคิวทั้ง bot · กันตอบซ้ำตอนถูกบล็อกด้วย dedupe update_id · short polling เสมอ (`timeout=0`) + กัน 409 (ลองใหม่ → ลบ webhook → ถอย) + 429 flood wait (รอ `retry_after`) — กัน bot lock จากหลายโปรแกรมใช้ bot เดียวกัน
- **คำตอบจากคำสั่ง Telegram ขึ้นต้นด้วย `[ชื่อเครื่อง]`** — รู้ว่าคำตอบมาจากเครื่องไหน (ใช้ bot กลางหลายเครื่อง) — ได้ทั้งตอบตรงและผลลัพธ์ `/run` ที่ตอบทีหลัง
...
- **เช็คเวอร์ชันใหม่ตอนเริ่มโปรแกรม/service + ทุก 1 ชม.**: รันยาว ๆ ก็รู้ว่ามีเวอร์ชันใหม่ — log + แจ้ง Telegram 1 ครั้งต่อเวอร์ชัน (เฉพาะที่ตั้งค่า Telegram ไว้)
- **ฟอร์มรหัสผ่านหน้าเว็บไม่โชว์ hash**: ช่องว่างเสมอ — เว้นว่าง+บันทึก = คงรหัสเดิม · พิมพ์ใหม่ = เปลี่ยน · ปุ่ม "ลบรหัส" = ลบ (เข้าเว็บไม่ต้อง login)
